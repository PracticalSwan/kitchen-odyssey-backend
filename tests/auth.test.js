...
const { generateAccessToken, generateRefreshToken, verifyToken, setAuthCookies, clearAuthCookies } = await import('@/lib/auth.js');
...
describe('auth.js — token utilities', () => {
  ...
  describe('setAuthCookies', () => {
    it('sets access, refresh, and csrf Set-Cookie headers on response', () => {
      ...
      expect(cookies.some(cookie => cookie.includes('ko_access=access-tok'))).toBe(true);
      expect(cookies.some(cookie => cookie.includes('ko_refresh=refresh-tok'))).toBe(true);
  ...
  describe('clearAuthCookies', () => {
    it('sets Max-Age=0 on access, refresh, and csrf cookies', () => {
      ...
      expect(cookies.some(cookie => cookie.includes('ko_access='))).toBe(true);
      expect(cookies.some(cookie => cookie.includes('ko_refresh='))).toBe(true);
      expect(cookies.some(cookie => cookie.includes('ko_csrf='))).toBe(true);
    });
  });
});

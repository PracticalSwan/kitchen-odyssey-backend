...
export async function GET(request, { params }) {
  ...
  try {
    ...
    if (recipe.status !== "published") {
      ...
      if (
        !authUser ||
        (authUser.userId !== recipe.authorId && authUser.role !== "admin")
      ) {
        return errors.notFound("Recipe not found", cors);
      }
    }
...
export async function PATCH(request, { params }) {
  ...
  try {
    ...
    if (body.prepTime !== undefined)
      recipe.prepTime = Number.isFinite(Number(body.prepTime))
        ? Math.max(0, Number(body.prepTime))
        : recipe.prepTime;
    if (body.cookTime !== undefined)
      recipe.cookTime = Number.isFinite(Number(body.cookTime))
        ? Math.max(0, Number(body.cookTime))
        : recipe.cookTime;
    if (body.servings !== undefined)
      recipe.servings = Number.isFinite(Number(body.servings))
    ...
    if (typeof recipe.title === "string")
      recipe.title = sanitizeString(recipe.title, 100);
    if (typeof recipe.description === "string")
      recipe.description = sanitizeString(recipe.description, 1000);
    if (typeof recipe.category === "string")
      recipe.category = sanitizeString(recipe.category, 100);
    if (typeof recipe.imageAltText === "string")
      recipe.imageAltText = sanitizeString(recipe.imageAltText, 200);
